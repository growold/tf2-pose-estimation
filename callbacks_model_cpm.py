...
def get_check_pointer_callback(model_path, output_name):
    checkpoint_path = os.path.join(
        model_path, output_name + ".hdf5")  # ".ckpt"
...
def get_img_tensorboard_callback(log_path, output_name, inputs, labels, model):
    # tensorboard image
    ...
    file_writer = tf.summary.create_file_writer(
        os.path.join(log_path, output_name))
    ...
    def log_tensorboard_predicted_images(epoch, logs):
        ...
        for i in range(images.shape[0]):
            ...
            numpy_img = PoseImageProcessor.display_image(
                image, true_heat=label, pred_heat=prediction, as_numpy=True)
        ...
        with file_writer.as_default():
            ...
            tf.summary.image("predict from validation dataset",
                             predicted_images, max_outputs=10, step=epoch)

    # Define the per-epoch callback.
    img_tensorboard_callback = tf.keras.callbacks.LambdaCallback(
        on_epoch_end=log_tensorboard_predicted_images)
